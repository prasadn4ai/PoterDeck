...
export function buildPptx(slides, style, colorTheme, deckTitle, options = {}) {
  const { watermark = true, logoData = null, logoPosition = 'bottom-left' } = options;
  ...
  for (const slide of slides) {
    ...
    // Add watermark to every slide (free tier)
    if (watermark) {
      pptxSlide.addText('Made with PoterDeck', {
        x: 8.5, y: 5.15, w: 4.0, fontSize: 8,
        fontFace: font, color: 'BBBBBB', align: 'right', italic: true,
      });
    }
    ...
    if (logoData && logoData.startsWith('data:image')) {
      const logoPos = {
        ...
        'top-right': { x: 12.0, y: 0.2 },
        'bottom-left': { x: 0.2, y: 4.8 },
        'bottom-right': { x: 12.0, y: 4.8 },
      };
      const pos = logoPos[logoPosition] || logoPos['bottom-left'];
      try {
        pptxSlide.addImage({ data: logoData, x: pos.x, y: pos.y, w: 0.8, h: 0.8 });
